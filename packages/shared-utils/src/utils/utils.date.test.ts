...
describe('dateUtils', () => {
  describe('parseEventDate', () => {
    ...
    it('영어 포맷: 다른 해', () => {
      ...
      const date = new Date('2024-03-02T05:30:00Z');
      expect(dateUtils.parseEventDate(date, { formatStyle: 'english' })).toBe('Mar 02, 2024, 2:30 PM');
    });
  });
  ...
  describe('toUTCDayRangeFromYYYYMMDD', () => {
    it('KST 자정 → UTC 범위 반환 (20250101)', () => {
      ...
      const [utcStart, utcEnd] = dateUtils.toUTCDayRangeFromYYYYMMDD({ yyyymmdd: '20250101' });
    ...
    it('KST 자정 → UTC 범위 반환 (20250315)', () => {
      ...
      const [utcStart, utcEnd] = dateUtils.toUTCDayRangeFromYYYYMMDD({ yyyymmdd: '20250315' });
  ...
  describe('getWeekendUTCStartDates', () => {
    it('항상 FRIDAY / SATURDAY / SUNDAY 3개 범위 반환', () => {
      const result = dateUtils.getWeekendUTCStartDates({ yyyymmdd: '20250101' });
    ...
    it('금요일 기준: offset 0 → 같은 금요일 반환', () => {
      ...
      const result = dateUtils.getWeekendUTCStartDates({ yyyymmdd: '20250103' });
      expect(result[0].utcStart.toISOString()).toBe('2025-01-02T15:00:00.000Z');
    });
    ...
    it('토요일 기준: offset 6 → 다음 주 금요일 반환', () => {
      ...
      const result = dateUtils.getWeekendUTCStartDates({ yyyymmdd: '20250104' });
    ...
    it('일요일 기준: offset 5 → 같은 주 금요일 반환', () => {
      ...
      const result = dateUtils.getWeekendUTCStartDates({ yyyymmdd: '20250105' });
      expect(result[0].utcStart.toISOString()).toBe('2025-01-09T15:00:00.000Z');
    });
