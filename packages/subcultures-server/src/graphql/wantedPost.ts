...
export const resolvers: IResolvers<any> = {
    Query: {
        wantedPost: async (
            ...
            try {
                const query = await getRepository(WantedPost)
                    ...
                    .leftJoinAndSelect(
                        'wanted_post.bookmarks',
                        'wantedPostBookmarks'
                    )
                    .leftJoinAndSelect(
                        'wantedPostBookmarks.user',
                        'bookmarkUser'
                    )
        ...
        wantedPostList: async (
            ...
            try {
                ...
                if (args.category) {
                    const category = await getRepository(
                        WantedPostCategory
                    ).findOne({
                        where: {
                            value: args.category,
                    ...
                    if (category) {
                        query.where(
                            'wanted_post.fk_category_id = :category_id',
                            {
    ...
    Mutation: {
        createWantedPost: async (
            ...
            if (!user) {
                return new AuthenticationError(
                    'createWantedPost - user is not defined'
                )
            }
            try {
                const category = await getRepository(
