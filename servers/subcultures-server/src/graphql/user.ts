...
export const typeDef = gql`
  ...
  extend type Mutation {
    ...
    loginWithEmail(email: String!, password: String!): LoginWithEmailResponse
      ...
      try {
        ...
        return {
          ...
          token: canBeExpiredSoon
            ? generateToken((response as DecodeTokenResponse).userId)
            : token,
        }
      } catch (e) {
        console.error(e)
  ...
  Mutation: {
    ...
    sendAuthcodeEmail: async (
      ...
      try {
        ...
        if (!messageId) {
          throw Error('createUserWithEmail - messageId is not defined')
        }
        if (existing) {
    ...
    createUserWithSocial: async (
      ...
      try {
        ...
        while (unavailableUsername) {
          ...
          const existingUserWithUsername = await getRepository(User).findOne({
            where: {
              username,
            },
          })
        ...
        const existingUserWithSocialInfos = await getRepository(User).findOne({
          where: {
            socialId,
            socialCompany,
          },
    ...
    eraseAccount: async (
      ...
      try {
        ...
        if (wantedPostBookmarks.length > 0) {
          await getRepository(WantedPostBookmark).remove(wantedPostBookmarks)
        }
