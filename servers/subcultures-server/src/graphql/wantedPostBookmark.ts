...
export const resolvers: IResolvers<any> = {
  Query: {
    myWantedPostBookmark: async (parent: any, args: {}, ctx: ApolloContext) => {
      const { user } = ctx
      if (!user) {
      ...
      try {
        ...
        const query = await getRepository(WantedPostBookmark)
          ...
          .leftJoinAndSelect('wanted_post_bookmark.wantedPost', 'wantedPost')
  ...
  Mutation: {
    createWantedPostBookmark: async (
      ...
      try {
        ...
        const existing = await getRepository(WantedPostBookmark).findOne({
          where: {
            fk_user_id: userId,
        ...
        const created =
          await getRepository(WantedPostBookmark).save(wantedPostBookmark)
        return created
      } catch (e) {
        console.error(e)
        return new ApolloError(
          `createWantedPostBookmark - ${e}`,
          'INTERNAL_SERVER_ERROR'
        )
      }
    },
